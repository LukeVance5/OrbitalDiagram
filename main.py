...
def tick(bodies):
    for body1 in bodies:
        ...
        for body2 in exclude_body(body1,bodies):
            ...
            F_x += -(G*body1.mass*body2.mass*(x_dif/((x_dif**2+y_dif**2)**(1/2)))/(x_dif**2+y_dif**2))
            F_y += -(G*body1.mass*body2.mass*(y_dif/((x_dif**2 + y_dif**2)**(1 / 2)))/(x_dif**2 +y_dif**2))
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                bodies.append(Celestial_Body(("Planet" + str(len(bodies))), random.randint(10,1000),random.randint(1,5),((random.randint(0,255),random.randint(0,255),random.randint(0,255))),pos[0],pos[1],random.randint(1,100),random.randint(1,100),[]))
        tick(bodies)
        draw(bodies)
